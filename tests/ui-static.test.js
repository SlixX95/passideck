...
assert.ok(server.includes("const sequence = replay.kind === 'tmux-history' ? replay.clientWritten : ws.hydrationStartSequence") && server.includes("ws.finishHydration?.('live output during hydration')") && !server.includes('ws.hydrationOutput') && server.includes("ws.finishHydration?.('pty exited during hydration')"), 'hydration must replay from an immutable boundary and fall back immediately when live PTY output arrives');
...
assert.ok(service.includes('CPUWeight=200') && server.includes('const PASSIDECK_CPU_WEIGHT = 200') && server.includes('`CPUWeight=${PASSIDECK_CPU_WEIGHT}`') && server.includes('prioritizeTmuxScope(session)'), 'PassiDeck must prioritize its bridge and owned tmux pane scopes inside Maeve');
...
assert.ok(
  ...
  terminalTouchEndBlock.includes('clearResponseAttention(session.id)') &&
  terminalTouchBlock.includes("sendApplicationMouse(0, touch, true)"),
  'Hermes TUI touch gestures must bypass text drag-selection, acknowledge a completed tap, and enter xterm through its wheel owner'
);
assert.ok(
