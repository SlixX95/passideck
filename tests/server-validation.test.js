...
(async () => {
  try {
    ...
    hermesDb.prepare('INSERT INTO sessions (id, source, started_at, title) VALUES (?, ?, ?, ?)').run('hermes-switch', 'passideck:former-pane', 1.5, 'Other Session');
    syncHermesTitles(app.sessions, hermesDb, () => 'hermes-switch');
    syncHermesTitles(app.sessions, hermesDb, () => 'hermes-1');
    syncHermesTitles(app.sessions, hermesDb, () => 'hermes-switch');
    assert.strictEqual(hermesEvents.filter(event => event.type === 'response-complete').length, 1, 'switching between Hermes sessions must not replay old completions');
    hermesDb.prepare('UPDATE messages SET active = 0 WHERE id = 2').run();
    syncHermesTitles(app.sessions, hermesDb, () => 'hermes-1');
    hermesDb.prepare('UPDATE messages SET active = 1 WHERE id = 2').run();
    syncHermesTitles(app.sessions, hermesDb, () => 'hermes-1');
    assert.strictEqual(hermesEvents.filter(event => event.type === 'response-complete').length, 1, 'message compaction must not rewind completion deduplication');
    syncHermesTitles(app.sessions, hermesDb, () => 'hermes-1');
